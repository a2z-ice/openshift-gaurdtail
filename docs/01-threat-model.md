...
| S3 | A PR removes a critical manifest from Git and Argo CD prunes it | medium | `prune: false` on the guardrails app (L2); Argo CD SA is **not** exempt so the prune is denied anyway (L3); 2-reviewer ruleset (L8) |
| S4 | Compromised or malicious cluster-admin tries to delete / cripple Argo CD | low, high impact | VAP applies to `system:masters` (L3); forging approvals needs impersonation → `ImpersonationUsed` P1 (L6); no standing cluster-admin for humans (L1) |
| S5 | Attacker first disables the guardrail (delete binding, edit `GuardrailConfig`, set `validationActions: []`) | low | Policy objects are critical by name (L3+L4); RBAC: only Argo CD SA + break-glass may write them (L1); Argo CD self-heal (L2); `GuardrailPolicyModified` P1 (L6) |
...
| L2 | Argo CD native: no cascade finalizers, `prune: false`, `selfHeal`, AppProject fencing, Argo CD RBAC (operators cannot delete apps) | S2, S3, S5 |
| L3 | Admission: `guardrails-critical-delete` (two-person rule), `guardrails-gitops-only-mutation`, `guardrails-rbac-escalation-audit` | S1–S7 |
...
| L6 | Detection: 15 Loki rules on the audit stream + 10 metrics rules, two independent pipelines | S1–S7, S9 |
...
|---|---|---|---|---|
| R1 | cluster-admin impersonates an approver to forge approvals | Security | `ImpersonationUsed` P1; JIT cluster-admin; audit shows `impersonatedUser` so forgery is provable | quarterly |
...
| R4 | Approval TTL is enforced by the reaper CronJob, not by admission (CEL has no clock). A stale approval on a specific object could be used later for **that object only** | Platform | reaper every 10 min; `GuardrailReaperFailing` alert; approvals are wiped whenever the request text/author changes | quarterly |
