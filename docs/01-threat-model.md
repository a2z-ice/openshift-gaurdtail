...
| S4 | Compromised or malicious platform admin tries to delete / cripple Argo CD | low, high impact | VAP applies to every identity (L3); impersonating an approver, break-glass or the Argo CD controller no longer works for writes (docs/17, L1+L3); minting their tokens → `PrivilegedTokenMinted` P1 (L6); no standing admin membership for humans (L1) |
...
|---|---|---|---|---|
| R1 | Impersonation of an approver, break-glass or an Argo CD controller | Security | **Closed for every human role** (docs/17): no human holds `cluster-admin`; `impersonate` is granted on users/groups/serviceaccounts only (never `userextras`); exemptions and approver status require the authentication markers a real session carries; non-dry-run impersonated writes are denied (`guardrails-impersonation-dry-run-only`) and alerted (`ImpersonatedWriteDenied`). Remaining: the `system:masters` installer certificate (vaulted, alerted) and token minting for privileged SAs (`PrivilegedTokenMinted`, custodian Role) | quarterly |
