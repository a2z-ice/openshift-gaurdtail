...
| R2 | "Critical resource should not be deleted by a single user even if it is the cluster admin" | Insider error and insider threat at the highest privilege level; `cluster-admin` is `*` on `*`, RBAC cannot restrain it | `manifests/03-guardrails/vap-critical-delete.yaml`, docs/04 |
...
7. **Everything is code, everything is tested.** Manifests, policies, alert rules, scripts, CI invariants and a 30-case test matrix are in one repository under a ruleset with no bypass.
...
**Decision.** (1) The `ArgoCD` CR, both namespaces, the operator's Subscription/CSV/OperatorGroup, every `argoproj.io` CRD, AppProjects and Applications are critical, most of them by name so they are protected even before a label could be applied. (2) The guardrails Application has no cascade finalizer, `prune: false`, `selfHeal: true`; ApplicationSets get `preserveResourcesOnDeletion`; Argo CD RBAC denies `delete` on applications/projects to operators. (3) The Argo CD controller service account is deliberately **not** exempt: if a PR removes a critical manifest, the prune is denied by the same policy and alerted. Argo CD is also the mechanism that heals the control: any out-of-band change to a policy object is reverted on the next reconcile.

**Why not the alternatives.** Exempting the Argo CD SA would make "merge a PR that deletes the policy" a one-person path with only Git-side review as protection. Protecting only by label would leave the namespace and CRDs, which cannot be labelled by Git before they exist, unprotected.
...
**Decision.** Five layers: the policy objects, bindings, `GuardrailConfig`, `guardrails-*` RBAC and approver groups are critical by name; only the Argo CD SA and break-glass hold write RBAC on them; Argo CD self-heals them; every write by anyone else raises `GuardrailPolicyModified` (P1); the only legitimate path is a PR with two approvals and code-owner review under a ruleset with no bypass actors, checked by CI invariants that reject weakening (humans in `exemptUsers`, `minApprovers < 2`, `Deny` removed, finalizer/prune added, plaintext secrets).
...
**Decision.** Four scripts for the workflow with pre-checks and confirmations; a verify script for health; a 30-case automated matrix producing an evidence log; runbooks for deletion, red alert, tampering, pipeline failure, break-glass, restore; recurring activities and KPIs; control mapping to ISO 27001, SOC 2, NIST 800-53, CIS, PCI DSS; an AI-assistant layer (`AGENTS.md`, skills, subagents, shared memory) so any operator or tool can perform the procedures consistently.
...
| Exempt the Argo CD service account | Would turn "merge a PR" into a single-person deletion path |
...
| Statement | Status |
|---|---|
| Manifests build (5 kustomizations), lint clean, scripts parse, CI invariants pass, diagrams parse | **Proven locally** (docs/11) |
...
| Three colluding people, control-plane node access, IdP compromise | **Residual**, with compensating controls in docs/01 |
