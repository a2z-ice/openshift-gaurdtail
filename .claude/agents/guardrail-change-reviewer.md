...
1. Weakening: humans in `exemptUsers`; `minApprovers < 2`; `executorMayBeApprover: true`; `Deny` removed from phase-3/4 bindings; `failurePolicy` changed from `Fail`; `parameterNotFoundAction` changed from `Deny` on the deletion bindings; kinds removed from `matchConstraints`; name-protected list shortened.
2. Argo CD: `resources-finalizer.argocd.argoproj.io` or `prune: true` on `application-guardrails.yaml`; `selfHeal` disabled; AppProject whitelist widened.
