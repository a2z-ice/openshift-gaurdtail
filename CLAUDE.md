...
for o in manifests/base manifests/overlays/*; do kustomize build "$o" >/dev/null && echo "OK $o"; done   # expect 92 objects each
...
5. **`vap-impersonation-dry-run-only.yaml`** (impersonated writes only with `--dry-run=server`; identity markers in `userInfo.extra`), **`vap-critical-label-control.yaml`** (who may add the critical label), **`vap-gitops-only-mutation.yaml`** (binding `guardrails-gitops-only-mutation` is Audit until phase 4; binding `-hardened` is Deny in every phase for the self-protection set, which includes the critical Argo CD objects: patching the guardrails Application's `spec.source.path` would otherwise silently move the cluster back to phase 1 (docs/20 G9)) and **`vap-rbac-escalation-audit.yaml`** (never denies; its "failure" stamps the audit event) reuse the same variable patterns.
