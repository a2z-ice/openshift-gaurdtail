...
for o in manifests/base manifests/overlays/*; do kustomize build "$o" >/dev/null && echo "OK $o"; done   # expect 60 objects each
...
scripts/test-guardrails.sh                      # 32-case matrix in scratch ns guardrails-test; writes evidence-<ts>.log
...
3. **`vap-critical-delete-bindings.yaml`**: binding `-labelled` uses an `objectSelector` (evaluated against object OR oldObject), binding `-named` lists low-traffic kinds without a selector. This scoping is why ordinary Secret/ConfigMap traffic is not evaluated and why a missing `GuardrailConfig` fails closed only for critical objects.
4. **`vap-gitops-only-mutation.yaml`** (Audit until phase 4) and **`vap-rbac-escalation-audit.yaml`** (never denies; its "failure" stamps the audit event) reuse the same variable patterns.
5. **`reaper-cronjob.yaml`**: CEL has no clock, so approval TTL is a CronJob whose SA the policy allows to *remove* entries only.
6. **`manifests/02-rbac`** grants approvers `patch` (RBAC cannot restrict to annotations; the policy does that), executors `delete`, and binds the break-glass SA. **`manifests/00-namespaces-and-groups`** are the bootstrap groups (IdP-synced in production).
7. **`manifests/01-audit`** turns on `WriteRequestBodies` and forwards audit events to Splunk and LokiStack from one pipeline. **`manifests/05-alerting/loki-alertingrules-audit.yaml`** matches on the flattened field names (`annotations_guardrails_critical_delete_decision`, `..._validation_failure`), **`prometheusrule-gitops-health.yaml`** is the audit-independent second pipeline, and **`alertmanager-main.yaml`** routes `guardrail="true"` to email + Teams with `group_wait: 0s`.
8. **`manifests/04-argocd`**: the ArgoCD CR patch (SSO, RBAC deny delete, ignoreDifferences for the approval annotations), the fenced AppProject, and the app-of-apps with no cascade finalizer and `prune: false`.
9. **`scripts/lib.sh`** holds the annotation contract used by the four workflow scripts; `scripts/test-guardrails.sh` encodes the expected allow/deny of every rule.

Invariants that span files and must be kept in sync when one changes: the annotation keys (policy ↔ `scripts/lib.sh` ↔ reaper ConfigMap ↔ Argo CD `ignoreDifferences`), the policy names (↔ Loki rule field names, which derive from the policy name not the prefix), the V1–V4 rule table in `docs/04`, the facts table in `AGENTS.md` and `llm/context.yaml`, the test-case count (30) in `docs/04`, `docs/10`, `docs/11`.
...
- `docs/14-manual-test-guide.md` T-numbers and `scripts/test-guardrails.sh` cases describe the same scenarios; update both.
