...
- 2026-09-19: Simulation doc (docs/20) found G9 (CRITICAL: `oc patch application guardrails spec.source.path` -> phase1 = permanent silent disable, because each overlay renders the app pointing at itself; fixed by adding critical argoproj.io kinds to the -hardened binding), G12 (stock `default` AppProject allowed any repo -> laundering through the exempt controller; locked it), G10/G11/G13. Now 92 objects, 44 alerts, 80 automated + 50 manual cases.
