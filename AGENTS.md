...
| Self-protection set | GuardrailConfig, policies/bindings, guardrails-* RBAC, privileged Groups, APIServer/OAuth, labelled CronJobs, **critical Argo CD objects** (Application/AppProject/ApplicationSet/ArgoCD - they select the phase): GitOps-only (`-hardened` binding, Deny in every phase); humans may only add approval annotations |
...
scripts/test-guardrails.sh                                # phase-aware matrix (80 cases), scratch ns guardrails-test, evidence/ dir; impersonation mode needs a system:masters credential (pre-prod)
...
| Simulate a deletion end to end, or prove forgery is impossible | `docs/20-deletion-simulation.md` |
